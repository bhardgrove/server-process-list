...
$ps = explode("\n", trim(shell_exec('ps axo pid,ppid,%cpu,pmem,user,group,args --sort %cpu | grep -v ps')));

if($view === 'poller_status') {
$time = time();
exec('ps aux | grep poller.php | grep -v grep', $poller);
if(empty($poller)) {
	$poller = 0;
}
else $poller = 1;
print $time.",poller_status,".$poller;
exit();
}
...
<style>
body{margin:25px;font:bold 16px arial; background-color:white;color:black;line-height:30px;}
h2{padding:10px;}
